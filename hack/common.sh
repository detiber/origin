...
function os::build::get_product_vars() {
  ...
  export OS_BUILD_LDFLAGS_FEDERATION_SERVER_IMAGE_NAME="${OS_BUILD_LDFLAGS_IMAGE_PREFIX}-federation"
...
function os::build::ldflags() {
  ...
  ldflags+=($(os::build::ldflag "${OS_GO_PACKAGE}/cmd/kubefed.defaultEtcdImage" "${OS_BUILD_LDFLAGS_FEDERATION_ETCD_IMAGE}"))
